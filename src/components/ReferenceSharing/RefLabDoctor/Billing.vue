<template>
  <section class="ref-billing-page common-list-page">
      ...
      <div class="total-table mb-16">
        <div class="table-outer">
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item, index) in totalBillData" :key="item">
                <td>{{ index + 1 }}</td>
                <td>{{ item.total_doctor }}</td>
                <td>{{ item.total_investigation }}</td>
    ...
    <Modal
      v-model:show="formVar.doctorBillModal"
      class="billing-modal"
      headerClasses="header-bg"
    >
      <template v-slot:header>
        <div class="title" showHeader="true">Payment</div>
        <div class="close-btn" @click.prevent="formVar.doctorBillModal = false">
          <icon-cross></icon-cross>
        </div>
      </template>
      <div class="date-row">
      ...
      <div class="total-table mb-16">
        <div class="table-outer">
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item, index) in doctorTotalBillData" :key="item">
                <td>{{ index + 1 }}</td>
                <td>{{ item.total_doctor }}</td>
                <td>{{ item.total_investigation }}</td>
                <td>{{ item.total_price }}</td>
...
//Modal Table
const totalBillData = reactive([
  {
    total_doctor: 2,
    total_investigation: 'Post Prandial Blood Glucose',
    total_price: 2000,
    total_share: 10,
    total_discount: 10,
    total_paid: 1000,
